...
for country in ASEAN:
    ...
    for year in years: 
        ...
        for i in range(0,len(months),3):
            ...
            aggregate_quaterly_shipments = export_aggregate_by_quarter["Number Shipments"].sum() + import_aggregate_by_quarter["Number Shipments"].sum()
            data.append([f"20{year} Q{(i+3)//3}",country, aggregate_quaterly_shipments])

    df = pd.DataFrame(data,columns=["Quarterly","Country","Aggregate Quarterly Shipments"])
        
    # print(df)

    df.to_csv(f"C:\ASI_UROP\IATA_Database\CargoIS_Data\Total_Imports_Exports_Quarterly\Total_In_Sheet\{country}_quarterly_aggregate_shipments.csv")